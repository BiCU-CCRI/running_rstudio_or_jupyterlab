...
# Specify the path to your working directory; can be $(pwd -P) if you run the script from your working directory
workdir=$(pwd -P)
...
cat 1>&2 <<END
To access the RStudio Server while in the CCRI network, cmd + click for Mac/ctrl + click for Windows the link or copy-paste this to your web browser:
http://${HOSTIP}:${PORT}
---------------------------------------------------------
To access the RStudio Server while using the CCRI VPN, first copy-paste this into your terminal:
ssh -N -f -L localhost:${PORT}:localhost:${PORT} $(whoami)@${HOSTIP}
and then cmd + click for Mac/ctrl + click for Windows the link or copy-paste this to your web browser:
http://localhost:${PORT}
END
